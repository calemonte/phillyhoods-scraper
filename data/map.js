...
const URL =
  "http://sceti.library.upenn.edu/Philaneighborhoods/neighborhoods.cfm#";
    ...
    for (let i = 0; i < hoodNodes.length; i++) {
      ...
      for (let k = 0; k < links.length; k++) {
        if (
          links[k]
            .getAttribute("href")
            .includes("http://sceti.library.upenn.edu/")
        ) {
...
scrape().then(hoods => {
      ...
      if (hoods[w].reports.length) {
        for (let z = 0; z < hoods[w].reports.length; z++) {
          if (hoods[w].reports[z] === thisReport.report.id) {
            ...
            thisReport.bibliographicData.regionNeighborhoods =
              hoods[w].neighborhoods;
          }
        }
      }
    }
  ...
  fs.writeFile(
    "./outputs/hoodsDataWithRegions.json",
