...
fs.readdir("./outputs/images", (error, files) => {
  try {
    ...
    // Loop over our array of unique ids and create a folder for each report.
    for (let j = 0; j < reports.length; j++) {

      if (shell.which("mkdir")) {
        shell.exec(
          `mkdir -p ./outputs/ocr-temp/${reports[j]}_OCR`
        );
      }
      console.log(`Made directory for ${reports[j]}`);

    }
  } catch (error) {
    console.log("Something went wrong with making the directory: " + error);
  }
